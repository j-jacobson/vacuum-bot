...
// Voltage measured when battery fully charged
const float voltageBatCharged = 7.4; 

// Resistors 
const int resistor1 = 47; //kOhms
const int resistor2 = 100; //kOhms

//PWM for the micro metal motors
const int pwmMax = 170;// for 12V  pwmMAx = 170, for 10V output  pwmMax = 140
const int pwmMin = 0;;// for 9V  pwmMin = 128

//Maximum distance of the sensor
const int maxSharp = 30;
...
void forwardMotors(int moveTime){  
  //Manipulate direction according the desired movement of the motors
   analogWrite(motor1Pin1, pwmMax);
   analogWrite(motor2Pin1, pwmMax-10);
   delay(moveTime*0.25);
   
   analogWrite(motor1Pin2, pwmMin); //PWM value wher 0 = 0% and 255 = 100%
...
void rightMotors(int moveTime){ 
   ...
   analogWrite(motor1Pin2, pwmMax); 
   analogWrite(motor2Pin1, pwmMax);
   analogWrite(motor2Pin2, pwmMin); 
   delay(moveTime);
}
...
void leftMotors(int moveTime){ 
   ...
   analogWrite(motor2Pin2, pwmMax+20); 
   delay(moveTime);
}
...
float  readBattery(int input){
  ...
  voltage = (((readInput*4.9)/1000) * resistor2) / resistor1;
  Serial.print(" Battery Voltage: ");
  Serial.print(voltage);
  Serial.print(" Battery Percentage: ");
  Serial.print((voltage/voltageBatCharged) * 100);
  Serial.print("%");
  return voltage;
  } 
...
void batteryControl(int input){
  ...
  if(v_battery <= 6){ //battery limit of discharge, Don't put this limit lower than 3V per cell. 
    control = false;
    }
}
/////////////////////////////////////////////////MAIN CODE//////////////////////////////
void loop(){
  ...
  if (control){
    ...
    if (sdSHARP(SD2)<=7 ){ 
    ...
    else if (bumperState1==0){ // Right Bumper Activated 
      counter = counter+2;
      backwardMotors(1000); //backward delay of 500ms
      leftMotors(500);
      Serial.print("  Turning Left ");
      forward = false;
      }
      
    else if (bumperState2==0){ // Left Bumper Activated
      counter = counter+2;
      backwardMotors(1000); //backward delay of 500ms
      rightMotors(500);
      Serial.print("  Turning Right ");
      ...
      if(counter>=3){ //Corner
        ...
        leftMotors(2000);
        counter = 0;
        Serial.print("  Corner Detected. Reversing and pivoting");
        forward = false;
        }
